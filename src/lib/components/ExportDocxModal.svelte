  ...
  async function handleExport() {
    ...
    try {
      ...
      const path = await saveFile({
        defaultPath: `${bookTitle}.docx`,
        filters: [{ name: "Word Documents", extensions: ["docx"] }],
      });

      if (!path) return;

      await api.exportDocx(bookId, path);
...
{#if isOpen}
  <div class="modal-backdrop" role="presentation" onclick={handleCancel} onkeydown={(e) => e.key === "Escape" && handleCancel()}>
    <div class="modal" role="dialog" aria-modal="true" tabindex="-1" onclick={(e) => e.stopPropagation()}>
      ...
      <p class="description">
        Your manuscript will be exported with the compile order you've set. Pages marked as excluded will be skipped.
      </p>
